...
const defaultProducts: Product[] = [
	{
		...
		ingredients: ['Tomate', 'Aceite de oliva', 'Cebolla', 'Ajo', 'Sal', 'Azúcar'],
		nutritionalInfo: {
			calories: '45 kcal',
			protein: '1.2g',
			carbs: '8.5g',
	...
	},
	{
		...
		ingredients: ['Tomate natural', 'Aceite de oliva virgen extra', 'Cebolla', 'Pimiento', 'Especias naturales'],
		nutritionalInfo: {
			calories: '52 kcal',
			protein: '1.5g',
			carbs: '9.2g',
	...
	},
	{
		...
		ingredients: ['Tomate', 'Aceite vegetal', 'Cebolla', 'Azúcar', 'Sal', 'Vinagre'],
		nutritionalInfo: {
			calories: '48 kcal',
			protein: '1.1g',
			carbs: '8.8g',
	...
	},
	{
		...
		ingredients: ['Tomate', 'Aceite de oliva', 'Albahaca', 'Orégano', 'Ajo', 'Sal marina'],
		nutritionalInfo: {
			calories: '42 kcal',
			protein: '1.3g',
			carbs: '7.9g',
...
const ProductDetails = memo(({ product, onClose }: { product: Product; onClose: () => void }) => {
	return (
		<div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
			<div className="bg-gray-900/95 backdrop-blur-md rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-800/50 shadow-2xl animate-in slide-in-from-bottom-4 duration-500">
				...
				<div className="sticky top-0 bg-gray-900/95 backdrop-blur-md border-b border-gray-800/50 p-6 flex justify-between items-center">
					...
					>
						<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-label="Cerrar">
							<title>Cerrar</title>
				...
				<div className="p-8 space-y-8">
					...
					{product.nutritionalInfo && (
						<div className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 rounded-2xl p-6 border border-gray-700/30">
							<h3 className="text-xl font-bold text-fruco-gold mb-6 text-center border-b border-fruco-gold/30 pb-3">Información Nutricional</h3>
							<div className="text-center mb-4">
								<span className="text-sm text-gray-400 uppercase tracking-wider">Por 100g de producto</span>
							</div>
							<div className="space-y-4">
								<div className="flex justify-between items-center py-3 border-b border-gray-700/30">
									<span className="text-gray-300 font-medium">Valor energético</span>
									<span className="text-white font-bold text-lg">{product.nutritionalInfo.calories}</span>
								</div>
								<div className="flex justify-between items-center py-3 border-b border-gray-700/30">
									<span className="text-gray-300 font-medium">Proteínas</span>
									<span className="text-white font-bold text-lg">{product.nutritionalInfo.protein}</span>
								</div>
								<div className="flex justify-between items-center py-3 border-b border-gray-700/30">
									<span className="text-gray-300 font-medium">Hidratos de carbono</span>
									<span className="text-white font-bold text-lg">{product.nutritionalInfo.carbs}</span>
								</div>
								<div className="flex justify-between items-center py-3">
									<span className="text-gray-300 font-medium">Grasas</span>
									<span className="text-white font-bold text-lg">{product.nutritionalInfo.fat}</span>
								</div>
							</div>
						</div>
					)}
					...
					{product.ingredients && (
						<div className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 rounded-2xl p-6 border border-gray-700/30">
							<h3 className="text-xl font-bold text-fruco-gold mb-6 text-center border-b border-fruco-gold/30 pb-3">Ingredientes</h3>
							<div className="text-center mb-4">
								<span className="text-sm text-gray-400 uppercase tracking-wider">En orden de mayor a menor cantidad</span>
							</div>
	...
	return (
		<button
			...
			className={`product-item relative bg-gradient-to-br from-gray-900/90 via-gray-800/80 to-gray-900/90 backdrop-blur-xl rounded-3xl overflow-hidden border border-gray-700/30 transition-all duration-700 cursor-pointer group hover:scale-[1.02] hover:-translate-y-2 w-full text-left ${
				...
				isSelected ? 'ring-2 ring-fruco-gold/60 shadow-2xl shadow-fruco-gold/30 scale-[1.02] -translate-y-2' : 'shadow-xl shadow-black/20'
			}`}
			style={{
		...
		>
				...
				{!isLoaded && isInView && (
					<div className="absolute inset-0 flex items-center justify-center bg-gray-800/60 backdrop-blur-sm z-20">
						<div className="relative">
							...
							<div className="absolute inset-0 w-10 h-10 border-3 border-transparent border-t-fruco-gold/60 rounded-full animate-spin" style={{ animationDirection: 'reverse', animationDuration: '1.5s' }} />
						</div>
					</div>
				)}
			</div>
			...
			{/* Contenido */}
			<div className="relative p-8 text-center bg-gradient-to-t from-gray-900/95 via-gray-900/80 to-transparent" style={{ willChange: 'transform' }}>
				...
				<div className="mt-6 flex items-center justify-center space-x-2 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-2 group-hover:translate-y-0">
					<span className="text-xs text-fruco-gold/80 font-medium tracking-wider uppercase">Ver detalles</span>
					<svg className="w-4 h-4 text-fruco-gold/80 transform group-hover:translate-x-1 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-label="Flecha derecha">
				...
				>
					{products.map((product) => (
						<ProductCard 
							...
							isHidden={selectedProduct !== null && selectedProduct.id !== product.id}
						/>
					))}
				</div>
